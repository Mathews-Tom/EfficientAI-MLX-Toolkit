...
from .strategies import MagnitudePruner, GradientPruner, StructuredPruner, UnstructuredPruner
from .scheduler import PruningScheduler, GradualPruningScheduler
from .utils import calculate_sparsity, create_pruning_mask, apply_pruning_mask
...
class MLXPruner:
    """
    Main pruner class that orchestrates different pruning methods.
    ...
    def _oneshot_pruning(self, training_data: Optional[Any] = None) -> Any:
        ...
        if self.config.method == PruningMethod.MAGNITUDE:
            ...
            self.pruning_masks = self._create_gradient_masks(self.config.target_sparsity, training_data)
        else:
            self.pruning_masks = self._create_magnitude_masks(self.config.target_sparsity)
    ...
    def _create_magnitude_masks(self, target_sparsity: float) -> Dict[str, mx.array]:
        ...
        for name, module in self.model.named_modules():
            if isinstance(module, nn.Linear) and self.config.should_prune_layer(name):
                ...
                if isinstance(weight, mx.array):
                    # Calculate magnitude based on criterion
                    if self.config.criterion.value == "l1":
                        magnitude = mx.abs(weight)
                    elif self.config.criterion.value == "l2":
                        magnitude = weight ** 2
                    else:
                        magnitude = mx.abs(weight)  # Default to L1
        ...
        # Concatenate all weights to find global threshold
        if self.config.global_magnitude_pruning:
        ...
        for i, (name, module, weight_shape) in enumerate(layer_info):
            weight = module.weight
            ...
            if self.config.global_magnitude_pruning and threshold is not None:
            ...
            masks[name] = mask.astype(mx.float32)
    ...
    def _create_gradient_masks(self, target_sparsity: float, training_data: Optional[Any] = None) -> Dict[str, mx.array]:
        ...
        if training_data is None:
            logger.warning("No training data provided for gradient-based pruning, falling back to magnitude")
    ...
    def _recovery_training(
        ...
        try:
            ...
            logger.info(f"Using {self.config.recovery_optimizer} optimizer with lr={self.config.recovery_lr}")
    ...
    def save_pruned_model(self, output_path: Union[str, Path]) -> None:
        """
    ...
    def _calculate_pruning_stats(self) -> None:
        ...
        self.pruning_stats.update({
            ...
            "parameters_removed_percent": ((original_params - pruned_params) / original_params) * 100,
        })
            ...
            for name, module in self.pruned_model.named_modules():
                if isinstance(module, nn.Linear):
                    ...
                    if isinstance(weight, mx.array):
                        ...
                        layer_zeros = mx.sum(mx.abs(weight) < 1e-8)
