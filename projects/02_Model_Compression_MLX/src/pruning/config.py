...
class PruningConfig:
    ...
    def __post_init__(self):
        ...
        if not 0.0 <= self.initial_sparsity <= self.target_sparsity:
            raise ValueError("initial_sparsity must be between 0 and target_sparsity")
        ...
        if self.end_epoch <= self.start_epoch:
            raise ValueError(
                f"end_epoch ({self.end_epoch}) must be greater than start_epoch ({self.start_epoch})"
            )
    ...
    def from_dict(cls, config_dict: dict[str, Any]) -> "PruningConfig":
        ...
        numeric_fields = [
            ...
            "recovery_weight_decay",
            "recovery_gradient_clipping",
        ]
    ...
    def calculate_sparsity_for_epoch(self, current_epoch: int, total_epochs: int) -> float:
        ...
        if self.schedule == PruningSchedule.ONESHOT:
            return (
                self.target_sparsity if current_epoch >= self.start_epoch else self.initial_sparsity
            )
            ...
            progress = (current_epoch - self.start_epoch) / (self.end_epoch - self.start_epoch)
            sparsity_increase = (self.target_sparsity - self.initial_sparsity) * (
                1 - (1 - progress) ** 2
            )
            return self.initial_sparsity + sparsity_increase
