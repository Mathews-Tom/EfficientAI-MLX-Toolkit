...
class CompressionBenchmark:
    ...
    def _benchmark_baseline(self, model: Any, tokenizer: Any) -> Dict[str, Any]:
        ...
        try:
            ...
            baseline_results.update(self._benchmark_inference_performance(
            ...
            if eval_data:
                baseline_results.update(self._measure_accuracy_metrics(
    ...
    def _benchmark_compression_method(
        ...
        try:
            ...
            # Performance benchmarking
            method_results.update(self._benchmark_inference_performance(
                compressed_model, tokenizer, method
            ))
            ...
            eval_data = self._get_evaluation_data()
            if eval_data:
                method_results.update(self._measure_accuracy_metrics(
                    compressed_model, tokenizer, eval_data
                ))
    ...
    def _get_evaluation_data(self) -> Optional[List[str]]:
            ...
            for method, method_data in method_results.items():
                ...
                if ("model_size_mb" in baseline and "model_size_mb" in method_data):
                    ...
                    if comp_size > 0:
                        ...
                        method_summary["size_reduction_percent"] = ((orig_size - comp_size) / orig_size) * 100
    ...
    def _generate_text_report(self, results: Dict[str, Any]) -> str:
        ...
        if "baseline_avg_inference_time_ms" in baseline:
            report_lines.append(f"Avg Inference: {baseline['baseline_avg_inference_time_ms']:.2f} ms")
        ...
        for method, method_summary in summary.items():
            ...
            if "compression_ratio" in method_summary:
                report_lines.append(f"Compression Ratio: {method_summary['compression_ratio']:.2f}x")
            if "size_reduction_percent" in method_summary:
                report_lines.append(f"Size Reduction: {method_summary['size_reduction_percent']:.1f}%")
            if "inference_speedup" in method_summary:
                report_lines.append(f"Inference Speedup: {method_summary['inference_speedup']:.2f}x")
            if "perplexity_increase_percent" in method_summary:
                report_lines.append(f"Perplexity Increase: {method_summary['perplexity_increase_percent']:.1f}%")
