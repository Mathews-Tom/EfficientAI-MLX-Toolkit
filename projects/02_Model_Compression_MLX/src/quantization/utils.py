...
def calculate_quantization_error(
    original_tensor: Any, quantized_tensor: Any, metric: str = "mse"
) -> float:
    """
...
def estimate_compression_ratio(
    config: QuantizationConfig, model_info: dict[str, Any] | None = None
) -> dict[str, float]:
    """
...
def validate_quantization_config(config: QuantizationConfig) -> list[str]:
    ...
    if config.weight_bits <= 2:
        messages.append(
            "Warning: Very aggressive weight quantization may cause significant accuracy loss"
        )
    ...
    # MLX is always available in this toolkit
...
def calculate_quantization_scale_and_zero_point(
    min_val: float, max_val: float, num_bits: int, symmetric: bool = False
) -> tuple[float, int]:
    """
...
def quantize_weights(
    weights: Any, num_bits: int = 8, symmetric: bool = True, per_channel: bool = True
...
def dequantize_weights(quantized_weights: Any, quantization_params: dict[str, Any]) -> Any:
    """
...
def analyze_quantization_sensitivity(
    model: Any, calibration_data: Any, layer_names: list[str] | None = None
) -> dict[str, float]:
    """
...
def create_quantization_report(
    ...
    # Add summary
    compression_ratio = quantization_stats.get(
        "actual_compression_ratio", config.get_compression_ratio()
    )
