...
from .methods import PostTrainingQuantizer, DynamicQuantizer, QuantizationAwareTrainer
...
class MLXQuantizer:
    ...
    def _post_training_quantization(self, calibration_data: Optional[Any] = None) -> Any:
    ...
    def _quantization_aware_training(self, training_data: Optional[Any] = None) -> Any:
        """Perform quantization-aware training (placeholder for future implementation)."""
        logger.info("Starting quantization-aware training")
        
        # For now, fall back to post-training quantization
        # TODO: Implement full QAT with fake quantization during training
        logger.warning("QAT not fully implemented, falling back to PTQ")
    ...
    def _quantize_linear_layer(self, layer: nn.Linear, layer_name: str) -> None:
        ...
        logger.debug(f"Quantized layer {layer_name}: {weight.shape} -> {self.config.target_bits} bits")
    ...
    def save_quantized_model(self, output_path: Union[str, Path]) -> None:
        """
    ...
    def _calculate_model_size(self, model: Optional[Any] = None) -> float:
