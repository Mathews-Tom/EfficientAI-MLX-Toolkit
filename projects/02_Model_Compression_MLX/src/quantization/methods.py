...
from .config import QuantizationConfig, CalibrationMethod
...
class PostTrainingQuantizer:
    ...
    def _collect_calibration_stats(self, model: Any) -> None:
        ...
        if not MLX_AVAILABLE:
            logger.warning("MLX not available, using placeholder calibration")
            return
...
def create_quantized_linear_layer(
    ...
    """
    if not MLX_AVAILABLE:
        raise RuntimeError("MLX is required for quantized layers")
...
def quantize_tensor(
    ...
    """
    if not MLX_AVAILABLE:
        raise RuntimeError("MLX is required for tensor quantization")
