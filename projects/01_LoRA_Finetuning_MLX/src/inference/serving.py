...
from inference.engine import InferenceResult, LoRAInferenceEngine
...
class GenerationRequest(BaseModel):
    ...
    max_length: int | None = Field(100, ge=1, le=1024, description="Maximum tokens to generate")
    temperature: float | None = Field(0.7, ge=0.0, le=2.0, description="Sampling temperature")
    top_p: float | None = Field(0.9, ge=0.0, le=1.0, description="Nucleus sampling parameter")
    top_k: int | None = Field(50, ge=1, le=100, description="Top-k sampling parameter")
    repetition_penalty: float | None = Field(1.1, ge=0.0, le=2.0, description="Repetition penalty")
    stop_tokens: list[str] | None = Field(None, description="Stop tokens")
...
class BatchGenerationRequest(BaseModel):
    ...
    max_length: int | None = Field(100, ge=1, le=1024)
    temperature: float | None = Field(0.7, ge=0.0, le=2.0)
    top_p: float | None = Field(0.9, ge=0.0, le=1.0)
    top_k: int | None = Field(50, ge=1, le=100)
    repetition_penalty: float | None = Field(1.1, ge=0.0, le=2.0)
...
class GenerationResponse(BaseModel):
    ...
    def from_inference_result(
        cls, result: InferenceResult, request_id: str, generation_params: dict[str, Any]
    ) -> "GenerationResponse":
...
class LoRAServer:
    ...
    def __init__(
        ...
        # Server state
        self.inference_engine: LoRAInferenceEngine | None = None
...
def create_fastapi_app(
    ...
    **server_kwargs,
    ...
    server = LoRAServer(
        model_path=model_path, adapter_path=adapter_path, config=config, **server_kwargs
    )
    ...
    def _format_prometheus_metrics(server_instance) -> str:
        """Format server metrics in Prometheus format."""
        stats = server_instance.inference_engine.get_stats()
        uptime = time.time() - server_instance.start_time

        return f"""# HELP lora_server_uptime_seconds Server uptime in seconds
...
lora_active_requests {len(server_instance.active_requests)}
"""
    ...
    async def metrics():
        ...
        metrics_text = _format_prometheus_metrics(server)
