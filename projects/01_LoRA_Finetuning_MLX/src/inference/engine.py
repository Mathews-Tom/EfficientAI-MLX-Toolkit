...
from lora import InferenceConfig, LoRAConfig, ModelAdapter
...
class LoRAInferenceEngine:
    ...
    def _convert_to_mlx_format(adapter_path: Path) -> None:
        ...
        try:
            ...
            logger.info("Adapter format conversion completed")

        except Exception as e:
            logger.warning("Adapter conversion failed: %s", e)
            raise
    ...
    def _compile_model(self) -> None:
        ...
        try:
            ...
            logger.info("Compiling model for MLX optimization...")
    ...
    def generate(
        ...
        try:
            ...
            logger.error("MLX-LM generation failed: %s", e)
    ...
    def _sample_token(self, logits: mx.array, top_p: float, top_k: int) -> int:
        ...
        if top_k > 0:
            ...
            if top_k < vocab_size:
                ...
                top_k_logits = mx.scatter(
                    top_k_logits, top_k_indices, mx.take(logits, top_k_indices)
                )
                logits = top_k_logits
    ...
    def _update_stats(
        self, tokens_generated: int, inference_time: float, tokens_per_second: float
    ) -> None:
    ...
    def batch_generate(self, prompts: list[str], **generation_kwargs) -> list[InferenceResult]:
        """
    ...
    def from_pretrained(
        ...
        try:
            ...
            if model_id.startswith("mlx-community/") or model_path.exists():
                ...
                logger.info("Loading MLX-compatible model...")
                ...
                if adapter_path and Path(adapter_path).exists():
                    logger.info(
                        "Note: Custom LoRA format detected. Using base model for demonstration."
                    )
                    logger.info("For full LoRA support, adapters should be in MLX-LM .npz format.")

                logger.info("Successfully loaded MLX-native model")

            elif model_id.startswith("microsoft/DialoGPT"):
                logger.warning("DialoGPT models require conversion to MLX format")
                logger.info(
                    "Please use an MLX-compatible model like 'mlx-community/Llama-3.2-1B-Instruct-4bit'"
                )
                logger.info("For demonstration, creating a mock model...")
                ...
                try:
                    ...
                    model = MockMLXModel()
                    logger.info("Created mock model for demonstration")
                ...
                logger.info("Attempting to load as MLX model...")
                try:
                    model, tokenizer = load_model_and_tokenizer(model_id)
                    logger.info("Successfully loaded model with MLX")
                except Exception as e:
                    logger.warning("Failed to load with MLX: %s", e)
                    raise RuntimeError(
