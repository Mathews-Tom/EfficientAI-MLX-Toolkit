...
class GridSearch(SearchStrategy):
    ...
    def _generate_grid(self) -> List[Dict[str, Any]]:
        ...
        ranks = np.linspace(space_dict["rank"][0], space_dict["rank"][1], self.grid_density, dtype=int)
        alphas = np.linspace(space_dict["alpha"][0], space_dict["alpha"][1], self.grid_density)
        lrs = np.logspace(np.log10(space_dict["learning_rate"][0]), 
                         np.log10(space_dict["learning_rate"][1]), self.grid_density)
