...
from .layers import LoRAAttention, LoRAConv1D, LoRAEmbedding, LoRALinear
...
class LoRAAdapter:
    ...
    def __init__(
        ...
        self.lora_layer: nn.Module | None = None
    ...
    def _create_lora_layer(self) -> None:
        ...
        if self.layer_type == "linear":
            ...
            if "Quantized" in self.original_layer.__class__.__name__:
                ...
                has_bias = (
                    hasattr(self.original_layer, "bias") and self.original_layer.bias is not None
                )
            elif hasattr(self.original_layer, "out_features") and hasattr(
                ...
                raise ValueError(
                    f"Cannot determine dimensions for layer {self.layer_name}: {type(self.original_layer)}"
                )
    ...
    def _copy_original_weights(self) -> None:
        ...
        if hasattr(self.lora_layer, "linear"):
            ...
            if hasattr(self.original_layer, "weight"):
                ...
                logger.warning(
                    "Cannot copy weights from quantized layer %s. Using random initialization.",
                    self.layer_name,
                )
        ...
        for layer_name, layer in self.model.named_modules():
            ...
            if should_adapt:
                try:
                    ...
                    logger.warning("Failed to create adapter for %s: %s", layer_name, e)
                    continue

        logger.info("Created %d LoRA adapters", len(self.adapters))
...
class ModelAdapter:
    ...
    def adapt_model(self) -> None:
        ...
        if self._is_adapted:
            logger.info("Model is already adapted")
            return

        logger.info("Adapting model with LoRA...")
    ...
    def print_adaptation_summary(self) -> None:
        ...
        try:
            ...
            trainable_params = sum(
                mx.prod(mx.array(p.shape)) for p in trainable_params_dict.values()
            )
        ...
        logger.info("LoRA Adaptation Summary")
        logger.info("Total parameters: %s", f"{total_params:,}")
        logger.info("Trainable parameters: %s", f"{trainable_params:,}")
        if total_params > 0:
            logger.info("Trainable ratio: %.4f%%", trainable_params / total_params * 100)
        else:
            logger.info("Trainable ratio: N/A")
        logger.info("LoRA rank: %d", self.config.rank)
        logger.info("LoRA alpha: %s", self.config.alpha)
        logger.info("Scaling factor: %.4f", self.config.scaling_factor)
        logger.info("Adapted modules: %d", len(self.adapter_manager.adapters))
    ...
    def save_adapters(self, save_path: str | Path) -> None:
        ...
        logger.info("LoRA adapters saved to %s", save_path)

    def load_adapters(self, load_path: str | Path) -> None:
        ...
        logger.info("LoRA adapters loaded from %s", load_path)
